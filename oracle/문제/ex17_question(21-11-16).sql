...
select
    *
from tblInsa
    where 12*basicpay>= 20000000 and city = '서울' and jikwi in ('과장','부장');
...
select 
    *
from tblInsa
    where city in ('서울','인천','경기') and ibsadate BETWEEN '2010-01-01' and '2013-12-31';
...
SELECT
    sum(salary)
from employees
    where job_id in ('ST_CLERK','SH_CLERK인');
...
SELECT
    to_char(
    sum(basicpay) , 'L999,999,999') as "총 급여"
from tblInsa
...
select
    to_char(avg(salary),'$999,999') as avg
from employees
    where upper(first_name) like '%NI%';
--46. tblInsa. 간부급(과장,부장)들의 평균 급여는?
select
    ROUND(avg(basicpay),1)
from tblInsa
...
select 
    name , '19'||SUBSTR(ssn,1,2)
from tblInsa
    order by '19'||SUBSTR(ssn,1,2) asc;
--51. tblInsa. 서울에 사는 여직원 중 80년대생 몇명인지?
select
    *
from tblInsa
    where city = '서울' and ssn like '%-2%' and substr(ssn,1,2) BETWEEN 80 and 89;
...
select
    count (
        case
            when pm10 between (select pm10_start from fine_dust_standard where std_name = '좋음') 
                and (select pm10_end from fine_dust_standard where std_name = '좋음') 
                and pm25 between (select pm25_start from fine_dust_standard where std_name = '좋음') 
                and (select pm25_end from fine_dust_standard where std_name = '좋음')
                then to_char(MEA_DATE,'dd')
         end
     ) as "좋음",
    count (
        case
        when pm10 between (select pm10_start from fine_dust_standard where std_name = '보통') 
            and (select pm10_end from fine_dust_standard where std_name = '보통') 
            and pm25 between (select pm25_start from fine_dust_standard where std_name = '보통') 
            and (select pm25_end from fine_dust_standard where std_name = '보통')
            then to_char(MEA_DATE,'dd')
        end
     ) as "보통",
     count (
        case
        when pm10 between (select pm10_start from fine_dust_standard where std_name = '나쁨') 
            and (select pm10_end from fine_dust_standard where std_name = '나쁨') 
            and pm25 between (select pm25_start from fine_dust_standard where std_name = '나쁨') 
            and (select pm25_end from fine_dust_standard where std_name = '나쁨')
            then to_char(MEA_DATE,'dd')
        end
     ) as "나쁨",
     count (
        case
        when pm10 between (select pm10_start from fine_dust_standard where std_name = '매우나쁨') 
            and (select pm10_end from fine_dust_standard where std_name = '매우나쁨') 
            and pm25 between (select pm25_start from fine_dust_standard where std_name = '매우나쁨') 
            and (select pm25_end from fine_dust_standard where std_name = '매우나쁨')
            then to_char(MEA_DATE,'dd')
        end
     ) as "매우나쁨"
from fine_dust
...
select
    DISTINCT(family)
from tblzoo
    where wing = 'n' and leg =0;
--65. tbltodo. 오전(0~11시)과 오후(12~23시)에 끝마친(completedate) 할일들의 개수를 각각 가져오시오.
select
    count(
        case
            when to_char(completedate,'HH24') BETWEEN '0' and '11' then 'check'
        end
        ) as "오전",
     count(
        case
            when to_char(completedate,'HH24') BETWEEN '12' and '23' then 'check'
        end
        ) as "오후"
from tbltodo;
...
select
    count(
        case
            when instr(upper(email),'NAVER.COM') <> 0 then 'check'
        end
    ) as "네이버",
     count(
        case
            when instr(upper(email),'DAUM.NET') <> 0 then 'check'
        end
    ) as "다음",
     count(
        case
            when instr(upper(email),'GMAIL.COM') <> 0 then 'check'
        end
    ) as "구글"
from tbladdressbook;
