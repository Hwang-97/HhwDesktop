...
select a.* from (select BUSEO,count(BUSEO) as "인원수" from tblInsa where ssn like '%-2%' GROUP BY BUSEO order by count(BUSEO) desc) a where rownum <=1;
-- 4. tblInsa. 지역별 인원수 (내림차순)순위를 가져오시오.(city, 인원수)
select
    rownum as "순위",a.*
from (select city,count(city)as "인원수" from tblInsa group by city order by count(city) desc) a;
-- 5. tblInsa. 부서별 인원수가 가장 많은 부서 및원수 출력.
SELECT
    a.*
from(select buseo,count(buseo)as "인원수" from tblInsa  group by buseo order by count(buseo) desc) a
    where rownum <= 1;
...
select a.* from(select * from tblInsa order by ibsadate asc)a where rownum <=5;
...
select * from tblInsa where buseo = (SELECT
                                        buseo
                                    from(select a.buseo , rownum as rnum from (select buseo from tblInsa group by buseo order by avg(basicpay) desc) a)
                                        where rnum=2);
